<?php
/**
 * Adda Collection — User Suspension & Ban Management Console
 * Location: /admin/ban.php
 */
...
try {
    ...
    // BAN / UNBAN Action Trigger Handler
    if (isset($_GET['action']) && isset($_GET['user_id'])) {
        $targetId = (int)$_GET['user_id'];
        $action = $_GET['action'];
        
        if ($action === 'ban') {
            $stmt = $pdo->prepare("UPDATE users SET is_banned = 1 WHERE id = ? AND role != 'admin'");
            $stmt->execute([$targetId]);
            $status_msg = "Node Protection Active: User ID #$targetId has been suspended.";
        } elseif ($action === 'unban') {
            $stmt = $pdo->prepare("UPDATE users SET is_banned = 0 WHERE id = ? AND role != 'admin'");
            $stmt->execute([$targetId]);
            $status_msg = "Access Restored: User ID #$targetId restriction removed.";
        }
    }

    // Live Grid Search Logic
    ...
    die("Database Security Handshake Failed: " . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>SECURE NODE — BAN CENTER</title>
...
<body class="bg-[#2C1A11] text-[#FCFAF6] antialiased min-h-screen flex flex-col">
    ...
    <header class="h-16 border-b border-[#EFE9DE]/10 bg-[#352116] px-6 flex items-center justify-between sticky top-0 z-50 shadow-md">
        <div class="flex items-center gap-2">
            ...
            <span class="text-xs uppercase tracking-widest font-bold font-mono text-white">SECURE NODE // ACCOUNT SUSPENSION COMMAND</span>
        </div>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <aside class="w-80 bg-[#352116] border-r border-[#EFE9DE]/10 flex flex-col justify-between p-4 space-y-4 shrink-0 select-none">
            ...
            <div class="grid grid-cols-2 gap-2 overflow-y-auto pr-1">
                ...
                <a href="ban.php" class="flex flex-col items-center justify-center p-3 text-center bg-[#2C1A11] border border-rose-500/40 rounded-xl transition group">
                    <div class="text-rose-400 mb-1.5"><i class="fa-solid fa-user-slash text-sm"></i></div>
                    <span class="text-[9px] font-bold uppercase tracking-wider text-white">Ban Users</span>
                </a>
            ...
            <div class="bg-[#2C1A11] p-4 rounded-2xl border border-[#EFE9DE]/5 text-center text-[10px] text-gray-400 font-mono">
                Firewall Enforcement Node Active
            </div>
        </aside>
        ...
        <main class="flex-grow p-6 overflow-y-auto space-y-6">
            ...
            <?php if(!empty($status_msg)): ?>
                <div class="bg-rose-950/40 border border-rose-500/30 text-rose-300 text-xs p-4 rounded-xl font-mono flex items-center justify-between">
                    <span><i class="fa-solid fa-shield-halved mr-2"></i> <?php echo $status_msg; ?></span>
                    <a href="ban.php" class="text-gray-400 hover:text-white text-[10px]">DISMISS</a>
                </div>
            <?php endif; ?>

            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-xl font-bold tracking-tight text-white uppercase">Security Enforcement Desk</h1>
                    <p class="text-xs text-[#8C6D58]">
                        Total network profiles listed under system visibility: <span class="text-white font-bold font-mono"><?php echo count($users); ?></span>
                    </p>
                </div>
                
                <form action="ban.php" method="GET" class="w-full sm:w-80 flex items-center bg-[#352116] border border-[#EFE9DE]/10 rounded-xl p-1">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search_query); ?>" placeholder="Search user to ban/unban..." class="w-full bg-transparent text-xs text-white placeholder-gray-500 px-3 py-2 outline-none">
                    <?php if($search_query): ?>
                        <a href="ban.php" class="text-gray-400 hover:text-white px-2 text-xs"><i class="fa-solid fa-xmark"></i></a>
                    <?php endif; ?>
                    <button type="submit" class="bg-[#2C1A11] hover:bg-rose-600 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition">
                    ...
                    <table class="w-full text-left text-xs font-light">
                        <thead class="bg-[#2C1A11] text-[9px] uppercase tracking-widest text-[#8C6D58] border-b border-[#EFE9DE]/5">
                            <tr>
                                ...
                                <th class="p-4">Customer Network Identity</th>
                                <th class="p-4 text-center w-40">Access State</th>
                                <th class="p-4 text-right pr-6 w-56">Firewall Action Trigger</th>
                            </tr>
                        </thead>
                            ...
                            <?php if(empty($users)): ?>
                                <tr>
                                    <td colspan="4" class="p-8 text-center text-[#8C6D58] text-[10px] uppercase tracking-widest">
                                        No active nodes match the specified security parameters.
                                    </td>
                                </tr>
                            <?php else: ?>
                                ...
                                <tr class="hover:bg-[#2C1A11]/30 transition">
                                    ...
                                    <td class="p-4">
                                        ...
                                        <div class="text-[10px] text-gray-400 font-mono mt-0.5"><?php echo htmlspecialchars($user['email']); ?></div>
                                    </td>
                                    
                                    <td class="p-4 text-center">
                                        <?php if(isset($user['is_banned']) && $user['is_banned'] == 1): ?>
                                            <span class="bg-rose-950/50 text-rose-400 text-[9px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-md border border-rose-900/30 font-mono">
                                                BANNED / LOCKED
                                            </span>
                                        <?php else: ?>
                                            <span class="bg-emerald-950/50 text-emerald-400 text-[9px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-md border border-emerald-900/30 font-mono">
                                                CLEAR / ACTIVE
                                            </span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="p-4 text-right pr-6">
                                        <?php if(isset($user['is_banned']) && $user['is_banned'] == 1): ?>
                                            <a href="ban.php?action=unban&user_id=<?php echo $user['id']; ?>&search=<?php echo urlencode($search_query); ?>" class="inline-block bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-xl transition shadow-sm">
                                                <i class="fa-solid fa-unlock mr-1.5"></i> Lift Ban
                                            </a>
                                        <?php else: ?>
                                            <a href="ban.php?action=ban&user_id=<?php echo $user['id']; ?>&search=<?php echo urlencode($search_query); ?>" onclick="return confirm('Are you absolutely sure you want to ban this user profile?');\" class="inline-block bg-[#2C1A11] hover:bg-rose-600 hover:text-white border border-[#EFE9DE]/10 text-rose-400 text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-xl transition shadow-sm">
                                                <i class="fa-solid fa-user-slash mr-1.5"></i> Ban Profile
                                            </a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
